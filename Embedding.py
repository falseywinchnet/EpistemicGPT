...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.lm_head = ParallelSubspaceUnembed(config.n_embd, config.vocab_size,config.n_layer)
    ...
    def forward(self, idx, targets=None):
        ...
        if targets is not None:
            logits , lm_head_loss = self.lm_head(x)
            loss = self.criterion(logits, targets) + lm_head_loss * 0.001
        else:
